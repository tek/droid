...
//   lazy val sig = async.signalOf[Task, Option[A]](None).unsafeRun()

//   def view =
//     sig.continuous flatMap(_.fold[Stream[Task, A]](Stream())(Stream(_)))

//   def ios[C2] = view map(a => IO[A, C2](_ => a, a.toString))
...
//       def pure[A, C](run: C => A): StreamIO[A, C] =
//         StreamIO(IO(run, "ConsIO[StreamIO].pure"))
//     }
...
//         StreamIO(IO(c => f(fa.run(c)).io(c), "from Monad[StreamIO].flatMap"))
//       }
...
//   class Lifter[C]
//   {
//     def apply[A](f: C => A) = StreamIO(IO(f, "from StreamIO.Lifter"))
//   }
...
// case class ViewStream[A, C](view: Stream[Task, IO[A, C]])
// {
...
//       def pure[A](a: A) = ViewStream(Stream(IO[A, C](c => a, a.toString)))
...
//               "from Applicative[ViewStream].product")
//           }
//         }
//       }
...
// final class IOProcess[F[_, _]: PerformIO, A, C](self: Stream[Task, F[A, C]])
